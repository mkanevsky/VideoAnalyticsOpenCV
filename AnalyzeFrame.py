...
def AnalyzeFrame(frame):
    ...
    y,x = s[0], s[1]
    crop_img_side = frame[0:y, math.ceil(0.70*x):x]
    ...
    boundry_dict = {i:[min(x[0],x[6]) -5,max(x[2],x[4]) +5 ,min(x[3],x[1]) -5,max(x[5],x[7]) + 5] for i,x in enumerate(boundings.values())}
    # print(boundry_dict)
    ...
    boundry_temp3 = {0: [2.0, 40.0, 39.0, 69.0], 1: [2.0, 58.0, 62.0, 93.0], 2: [4.0, 48.0, 83.0, 109.0], 3: [1.0, 30.0, 106.0, 130.0], 4: [1.0, 26.0, 128.0, 151.0]}
